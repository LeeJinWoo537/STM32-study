...
## 😊 **자세하게 더 설명을 하자면**
ICM-20948_SPI_code.md 파일에 보면 우리가 NSS를 Disable로 해서 CS 직접 제어를 하게끔 설정을 하였다

#### 그러면 직접 제어는 어떻게 하는것이냐??
직접 제어는 정말 쉽다

```C
HAL_GPIO_WritePin(GPIOA, CS_PIN, GPIO_PIN_RESET);       // CS LOW 상태

// 이제 여기에 Transmit()나 Receive()나 TransmitReceive()로 사용하면 된다. 

HAL_GPIO_WritePin(GPIOA, CS_PIN, GPIO_PIN_SET);         // CS HIGH 상태
```
이런식으로 사용을 한다.

CS를 LOW로 해야만 센서를 설정을 할 수가 있다 그니까 좀 더 쉽게 이야기를 하자면

- `HAL_GPIO_WritePin(GPIOA, CS_PIN, GPIO_PIN_RESET);`: 자! 이제 나 너한테 말할거야 설정할거야!! 라고 하는 것이다.

그리고 센서를 다 읽었으면?!     <br>
HIGH로 읽기모드를 닫으면 된다!

- `HAL_GPIO_WritePin(GPIOA, CS_PIN, GPIO_PIN_SET);`: 자! 나는 이제 설정할거 다 했으니까 닫는다!

라고 생각하면 된다 그러면 LOW로 변환하고 그다음에 Transmit()나 Receive()나 TransmitReceive()로 사용해서 읽기를 하든 쓰기를 하든 한다음에 HIGH로 닫으면 된다.

### 🤔 **으잉?? 그러면 그냥 계속 LOW로 열면 안돼??**
엄밀히 말하면 안된다 그러면 센서는 계속 읽는다고 판단을 하고 자원을 낭비를 하게된다.