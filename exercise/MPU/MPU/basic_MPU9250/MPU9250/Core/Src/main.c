...
int main(void)
{
  ...
  int16_t accelData[3];  // 가속도 데이터 (X, Y, Z)
  int16_t gyroData[3];   // 자이로 데이터 (X, Y, Z)
  /* USER CODE END 2 */
...
void MPU9250_Read_Accel(int16_t *accelData)
{
    uint8_t data[6];  // 6바이트 데이터(X, Y, Z축 가속도)
    I2C_Read(MPU9250_ADDR, ACCEL_XOUT_H, data, 6);

    // 가속도 값은 16비트(2바이트)로 읽어온 후 합침
    accelData[0] = ((int16_t)(data[0] << 8 | data[1]));  // X축 가속도
    accelData[1] = ((int16_t)(data[2] << 8 | data[3]));  // Y축 가속도
    accelData[2] = ((int16_t)(data[4] << 8 | data[5]));  // Z축 가속도
}

// 자이로 데이터 읽기 함수
void MPU9250_Read_Gyro(int16_t *gyroData)
{
    uint8_t data[6];  // 6바이트 데이터(X, Y, Z축 자이로)
    I2C_Read(MPU9250_ADDR, GYRO_XOUT_H, data, 6);

    // 자이로 값은 16비트(2바이트)로 읽어온 후 합침
    gyroData[0] = ((int16_t)(data[0] << 8 | data[1]));  // X축 자이로
    gyroData[1] = ((int16_t)(data[2] << 8 | data[3]));  // Y축 자이로
    gyroData[2] = ((int16_t)(data[4] << 8 | data[5]));  // Z축 자이로
}
