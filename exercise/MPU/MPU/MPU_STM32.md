...
### 🤔 **궁금한게 MPU9250센서있잖아 보니까 레지스터 주소 값이 0xD0이더라고 근데 무슨 WHO_AM_I라는게 있는데 이건 0x71이래 그리고 또 뭐 0x75도 있고 이게 다 무슨 차이인거야??**
1. **0xD0 (I2C 주소)**
- 0xD0는 MPU9250의 I2C 주소입니다. I2C 주소는 MPU9250과 같은 장치가 통신할 때, 그 장치를 식별하는데 사용됩니다.
    - I2C 주소는 기본적으로 7비트 주소로 설정되어 있으며, 0x68 또는 0x69입니다.
    - 8비트 주소로 변환하면 0xD0이나 0xD1이 됩니다. (0xD0은 0x68의 왼쪽으로 시프트된 8비트 주소입니다.)
    - 이 주소는 슬레이브 주소로, MPU9250에 데이터를 읽거나 쓸 때 사용됩니다.

2. **0x71 (Who Am I 레지스터)**
- 0x71은 WHO_AM_I 레지스터의 값입니다.
    - 이 레지스터는 MPU9250이 정상적으로 작동하는지 확인할 수 있는 방법으로, 센서가 자신을 식별하는 고유한 값입니다.
    - WHO_AM_I 레지스터는 0x75 주소에서 읽을 수 있으며, 그 값은 0x71입니다. 즉, 이 값을 읽으면 0x71을 받게 됩니다.
    - WHO_AM_I 레지스터는 장치가 제대로 연결되었는지 확인할 때 사용합니다. 만약 이 값을 읽었을 때 0x71이 나오지 않으면, 장치가 제대로 연결되지 않았거나 문제가 있을 수 있습니다.

3. **0x75 (WHO_AM_I 레지스터 주소)**
- 0x75는 WHO_AM_I 레지스터의 주소입니다.
    - 이 레지스터에 접근하여, 해당 장치가 제대로 작동하고 있는지 확인할 수 있습니다.
    - 0x75 주소에서 값을 읽으면 0x71이 반환됩니다.
    - 즉, 0x75는 WHO_AM_I 레지스터의 주소이고, 그 레지스터에서 읽을 수 있는 값은 0x71입니다.

#### **요약:**
- 0xD0: MPU9250의 I2C 주소입니다.
- 0x75: WHO_AM_I 레지스터의 주소입니다.
- 0x71: WHO_AM_I 레지스터에서 읽을 수 있는 장치 고유 값입니다.

이 값들이 왜 중요한지:
- **I2C 주소(0xD0)**는 장치와 통신하기 위해 사용되고,
- WHO_AM_I (0x75) 레지스터는 장치가 제대로 연결되어 있는지 확인하는 역할을 합니다.
- 그 값이 0x71이므로, 이를 통해 MPU9250이 정상적으로 작동하고 있음을 알 수 있습니다.

<br>

### 🤔 **어? 그러면 WHO_AM_I는 뭐야?? 이건 뭔데 주소도 있고 고유에 값도 있는거야??**
### **WHO_AM_I란?**
WHO_AM_I는 주로 센서나 마이크로컨트롤러에서 제공하는 레지스터로, 장치가 자신을 식별할 수 있게 도와주는 역할을 합니다.   <br>
이 레지스터를 통해 장치가 자신의 고유 식별값을 전달할 수 있습니다.              <br>
예를 들어, MPU9250과 같은 센서에서 이 레지스터를 읽으면, 해당 센서가 제대로 작동하는지 확인할 수 있는 고유 값이 반환됩니다.

### **MPU9250의 WHO_AM_I 레지스터**
- **주소: 0x75** (이 주소에서 읽을 수 있습니다)
- **값: 0x71** (이 값을 읽으면 반환됩니다)

즉, 0x75 주소에 접근하면 0x71 값이 반환됩니다. 이는 MPU9250이 정상적으로 연결되어 작동하고 있음을 의미합니다.

### **왜 WHO_AM_I가 중요한가요?**
1. 장치 식별: 여러 개의 센서가 동일한 I2C 버스를 공유할 수 있기 때문에, 각 장치가 자신을 식별할 수 있게 해줍니다. 이 레지스터는 장치가 제대로 동작하는지 확인하는 간단한 방법입니다.
2. 통신 확인: WHO_AM_I 레지스터를 읽어보면, 센서가 정상적으로 연결되었는지 확인할 수 있습니다. 만약 값을 읽었을 때 0x71이 반환되지 않으면, 연결에 문제가 있거나 센서에 이상이 있을 수 있습니다.

### **예시:**
1. 0x75 주소에 접근: I2C 통신을 통해 0x75 주소에 접근합니다.
2. 값 읽기: 이 레지스터의 값은 0x71입니다. 따라서, 0x71을 읽었다면 센서가 정상적으로 작동하는 것입니다.

### ヾ(≧▽≦*)o 🎯 **쉽게말해서**
> **0x75는 MPU9250에 고유의 주소이고**      <br>
> **0x71은 MPU9250센서가 값을 읽으면 반환되는 값이고**  <br>
> **0x68(0xD0)은 I2C 주소이다.**

<br>

### 🤔 **근데 그러면 WHO_AM_I는 내가 지금 이걸 STM32 F411RE보드를 사용하고 있거든? 그리고 MPU9250센서를 사용하고 있고 근데 이게 STM32 그니까 보드에 있는거야? 아니면 MPU9250만 가지고 있는 주소값이야?? 이거 센서 말고 icm20948센서도 할거거든 근데 주소가 다른가 해서 그리고 저거 WHO_AM_I는 어떤보드에도 있는건가? 아니면 그 각 고유의 센서만 있는건가?**
### **결론부터 딱 말하면**
👉 **WHO_AM_I는 STM32 보드에 있는 게 아니고,**      <br>
**MPU9250 같은 “센서 IC 안에 들어있는 레지스터”입니다.**
- STM32 F411RE ❌
- MPU9250 ⭕
- ICM20948 ⭕ (근데 값은 다름)

### 1️⃣ **STM32 F411RE 보드 vs MPU9250 센서**
#### **STM32 F411RE**
- **마이크로컨트롤러(MCU)**
- I2C, SPI 같은 통신 마스터
- 센서한테 “야 너 누구냐?” 하고 물어보는 쪽
- ❌ WHO_AM_I 같은 개념 없음

#### **MPU9250**
- **센서 IC**
- 내부에 레지스터 맵이 있음
- 그 레지스터 중 하나가 WHO_AM_I
- ⭕ 자기 정체성을 알려줌
...
STM32 (마스터)
   ...
   |  I2C / SPI
   |
MPU9250 (슬레이브, WHO_AM_I 보유)
```

<br>

### 2️⃣ **WHO_AM_I가 정확히 뭐냐면**
> “내가 어떤 칩인지 확인해보라고        <br>
> 내가 고정값 하나 넣어둔 레지스터”
...
### 3️⃣ **그럼 ICM20948은?**
#### **ICM20948도 자기만의 WHO_AM_I 있음**
(회사도 같고 계열도 비슷하지만 칩은 다름)
- **WHO_AM_I 레지스터 주소:** `0x00`
- **값:** `0xEA`

즉:

| **센서 모델** | **WHO_AM_I 주소** | **WHO_AM_I 값** |
| :--- | :---: | :---: |
| MPU9250 | `0x75` | `0x71` |
| ICM20948 | `0x00` | `0xEA` |
...
### 4️⃣ **그럼 WHO_AM_I는 모든 보드/센서에 다 있나?**
- ❌ **아니요**

### **WHO_AM_I는:**
- 표준 규격 ❌
- “센서 제조사들이 관례처럼 넣어주는 레지스터” ⭕

그래서:
- 대부분의 IMU, 가속도, 자이로 센서엔 있음
- 하지만 **이름, 주소, 값 전부 다를 수 있음**
...
2. **WHO_AM_I 읽기**
3. 값 맞으면 → 초기화 진행
4. 값 다르면 → 배선 / 주소 / 센서 모델 확인

이거 안 되면 뒤에 아무 것도 의미 없음

### **한 줄 요약** 🔥
- **WHO_AM_I는 STM32 보드에 있는 게 아님**
- **각 센서 IC 내부에 있는 “신분증 레지스터”**
- 센서 바뀌면:
    - 주소 바뀔 수 있고
    - 값도 바뀜
- MPU9250 ≠ ICM20948 (완전 다름)

<br>

### 🤔 **이해했어 그러면 아까 0xD0 그니까 0x68이 I2C주소라고 했잖아 그러면 내가 예를 들어서 icm20948센서를 사용을할건데 근데 이 센서도 I2C로 사용을 할거야 그러면 주소값은 똑같이 0x68이야?? 아니면 또 센서마다 I2C주소값이 달라지는거야??**
**I2C 주소값은 센서마다 다를 수 있습니다.**         <br>
특히 MPU9250과 ICM20948처럼 서로 다른 센서들끼리는 I2C 주소가 다릅니다. 이 주소는 센서의 종류와 구성에 따라 달라져요.

### 1️⃣ **MPU9250의 I2C 주소 (기본값)**
- **I2C 주소: 0x68 또는 0x69**
    - 기본적으로 0x68이 많이 사용됩니다.
    - 0x69로 설정하려면 센서의 AD0 핀을 **High (3.3V)**로 설정해야 합니다.

<br>

### 2️⃣ **ICM20948의 I2C 주소**
- **I2C 주소: 0x68 또는 0x69**
    - **0x68**이 기본 주소입니다.
    - **0x69**로 설정하려면 **AD0 핀을 High (3.3V)**로 설정하면 됩니다.

**즉, ICM20948도 기본적으로는 0x68 주소를 가지지만, AD0 핀을 조정하여 0x69로 바꿀 수 있다는 점은 MPU9250과 동일합니다. 그러니까, 같은 주소값을 사용할 수 있습니다.**

하지만 주의할 점은:
- 두 센서의 기본 주소가 같더라도 센서 종류가 다르면 두 센서를 동시에 사용할 때 주소 충돌이 일어날 수 있습니다.
    - 예를 들어, I2C 통신에서 두 장치가 동일한 주소를 가질 경우 둘을 동시에 다룰 수 없게 됩니다. 이럴 경우 I2C 주소를 변경해서 각각 다른 주소로 바꿔야 합니다.

<br>

### 3️⃣ **I2C 주소 충돌 문제 해결**
1. **주소 변경 가능 센서:**
    - MPU9250과 ICM20948처럼 AD0 핀을 통해 주소를 변경할 수 있는 센서들은 주소 충돌을 피할 수 있습니다.
    - 보통 0x68과 0x69 두 주소가 기본으로 제공되기 때문에, 각 센서의 AD0 핀을 다르게 설정하면 충돌을 방지할 수 있습니다.

2. **주소 고정 센서:**
    - 일부 센서는 I2C 주소가 고정되어 있어 바꿀 수 없을 수도 있습니다. 이럴 경우 I2C 멀티플렉서를 사용하거나 다른 방법을 찾아야 합니다.

<br>

### 🤔 **어? 근데 너는 분명 I2C 주소는 센서마다 다를 수 있다고 했는데 MPU랑 ICM은 똑같은데? I2C 주소값이?**
**사실 MPU9250과 ICM20948의 I2C 주소는 기본값이 같습니다.**

그래서 좀 더 정확히 설명드리자면:

### 1️⃣ **MPU9250과 ICM20948의 I2C 주소가 같은 이유**
두 센서는 같은 계열에 속하는 센서들이기 때문에, **기본적으로 같은 I2C 주소**를 사용합니다. 구체적으로:
- **기본 I2C 주소: 0x68 또는 0x69**
    - **0x68**은 기본값이고, **AD0** 핀의 상태에 따라 **0x69**로 변경할 수 있습니다.

MPU9250과 ICM20948은 둘 다 InvenSense라는 회사에서 만든 센서이기 때문에, 기본 I2C 주소가 동일한 경우가 많습니다. 그러나 이 값이 동일해도 다른 센서일 때는 I2C 주소가 다를 수 있다는 점에서 일반적으로 I2C 주소는 다를 수 있다고 말씀드린 거예요.

<br>

### 2️⃣ **왜 I2C 주소가 같을 수 있나?**
- 계열이 같기 때문에: **MPU9250**과 **ICM20948**은 **InvenSense**라는 같은 회사의 제품이라서 기본 I2C 주소가 같을 수 있습니다.
- **AD0 핀**으로 주소 변경 가능: 기본적으로 **0x68** 주소를 사용하지만, **AD0** 핀을 **High**로 설정하면 **0x69**로 변경 가능합니다.

<br>

### 3️⃣ **센서마다 다를 수 있다는 의미:**
- 일반적인 규칙은 센서마다 I2C 주소가 다를 수 있다는 것입니다. 예를 들어, **MPU9250**과 **ICM20948**은 기본 주소가 **0x68**으로 동일하지만, 다른 종류의 센서들은 I2C 주소가 다를 수 있습니다. 예를 들어:
    - **BNO055 (자이로/가속도 센서)**는 기본 주소가 **0x28**
    - **L3GD20 (자이로 센서)**는 기본 주소가 **0x6A**

---------------------------------------------------------------------------------------------------------------------------------------

### 🤔 **궁금한게 저 MPU초기화 하는부분있잖아 init함수 거기서 0x00이 왜 NO Sleep인지 이게 무슨 의미인지 모르겠어 그리고 그밑에 Write()에서 0x6B가 있던데 이건 또 뭔지 모르겠어**
### **결론부터 말하면**
```c
HAL_I2C_Mem_Write(&hi2c1, MPU9250_ADDR, 0x6B, ...);
```
여기서 **`0x6B`는 MPU9250 내부에 있는 “레지스터 주소”**야.
> 👉 **PWR_MGMT_1 레지스터의 주소 = 0x6B**

### **감 잡기용 비유부터** 🧠
MPU9250을 아파트라고 생각해보자.
- `MPU9250_ADDR` → 아파트 **건물 주소**
- `0x6B` → 몇 동 몇 호
- `data` = `0x00` → 그 집에 넣어주는 **내용물**

즉,
> “MPU9250이라는 건물 안에 있는     <br>
> **0x6B번 방(PWR_MGMT_1)** 에          <br>
> **0x00이라는 값을 써라”**

라는 뜻이야.

### **HAL_I2C_Mem_Write 구조 뜯어보기**
```c
HAL_I2C_Mem_Write(
    &hi2c1,          // 어떤 I2C 포트?
    MPU9250_ADDR,    // 어떤 칩?
...
```
👉 **MPU9250 데이터시트에 정의된 레지스터 주소**

### **그럼 0x6B는 누가 정했냐?**
- ❌ **STM32가 정한 거 아님**
- ❌ **I2C 규칙 아님**

👉 **MPU9250 제조사(InvenSense)가 정함**

데이터시트 보면 이런 표가 있어 👇

| **레지스터 이름** | **주소 (Address)** | **설명 (비고)** |
| :--- | :---: | :--- |
| **WHO_AM_I** | `0x75` | 기기 식별 ID 확인 |
| **PWR_MGMT_1** | `0x6B` | 전원 관리 및 클럭 설정 |
| **PWR_MGMT_2** | `0x6C` | 축별(가속도/자이로) 활성화 |
| **GYRO_CONFIG** | `0x1B` | 자이로스코프 측정 범위 설정 |
| **ACCEL_CONFIG** | `0x1C` | 가속도계 측정 범위 설정 |
...
👉 **0x75 = WHO_AM_I 레지스터**
```c
HAL_I2C_Mem_Write(..., 0x6B, ..., &data, 1);
```
👉 **0x6B = PWR_MGMT_1 레지스터**

이제 패턴 보이지? 👀            <br>
**숫자가 의미 없이 나온 게 아니라 전부 “칩 내부 주소”야.**

### **왜 변수로 안 쓰고 0x6B를 그냥 쓰냐?**
보통은 이렇게 정의해 👇
```c
...
그러면
```c
HAL_I2C_Mem_Write(&hi2c1, MPU9250_ADDR,
                  MPU9250_PWR_MGMT_1,
                  1, &data, 1, HAL_MAX_DELAY);
```
이게 훨씬 **사람이 읽기 쉬운 코드지** 👍        <br>
(드라이버 만들 때는 거의 필수)

### **지금 단계에서 딱 기억하면 좋은 한 문장**
> **0x6B는 MPU9250 내부의 PWR_MGMT_1 레지스터 주소이고,**   <br>
> **그 주소에 0x00을 써서 슬립을 해제한다.**

<br>

### 🤔 **그러면 PWR_MGMT_1 이거에 대해서 알아야겠다 이게 뭐야??**
### **PWR_MGMT_1 이게 뭐냐면 한 줄 정의부터**
> **MPU9250의 “전원 + 동작 모드 컨트롤 센터” 레지스터**
...
| **비트 (Bit)** | **이름 (Name)** | **의미 (Description)** |
| :---: | :--- | :--- |
| **7** | DEVICE_RESET | `1` 설정 시 모든 레지스터를 기본값으로 리셋 |
| **6** | SLEEP | `1` 설정 시 저전력 잠자기 모드 (데이터 측정 안 함) |
| **5** | CYCLE | `1` 설정 시 SLEEP 모드 사이사이에 측정을 반복하는 주기 모드 |
| **4** | — | 사용 안 함 (Reserved) |
| **3 ~ 0** | CLKSEL | 클럭 소스 선택 (0: 내부 8MHz, 1: 자이로 축 참조 등) |


### 1️⃣ **DEVICE_RESET (bit 7)**
```c
data = 0x80; // 1000 0000
```
👉 **MPU9250을 소프트웨어로 리셋**
- 내부 레지스터 전부 초기값으로 돌아감
- 전원 다시 꽂은 것과 거의 동일
...
MPU9250은 **기본값이 SLEEP=1**이라서
이거 안 하면 센서가 아무 일도 안 함.
...
### 4️⃣ **CLKSEL (bit 3~0) ← 은근 중요**
> **“너 어떤 시계 보고 움직일래?”**

| **값 (Value)** | **이름 (Source)** | **의미 (Description)** |
| :---: | :--- | :--- |
| **0** | Internal 8MHz | 내부 오실레이터 사용 (정확도 낮음) |
| **1** | PLL with X Gyro | **X축 자이로 참조 (권장 ⭐)** |
| **2** | PLL with Y Gyro | Y축 자이로 참조 |
| **3** | PLL with Z Gyro | Z축 자이로 참조 |
| **7** | External | 외부 32.768kHz 클럭 소스 사용 |
...
### 🤔 **그러면 저 PWR_MGMT_1은 센서마다 하나씩 가지고 있는거야? 모든센서에 다있어? 아니면 STM32 보드에만 MCU보드에만 있는거야??**
### **결론 먼저 (이거만 기억해도 됨)**
> **`PWR_MGMT_1`은 MPU9250 센서 칩 내부에만 있는 레지스터다.**      <br>
> ❌ **STM32 보드에 있는 것도 아니고**              <br>
> ❌ **모든 센서에 공통으로 있는 것도 아니다.**
...
STM32 안에는        <br>
❌ MPU9250용 `PWR_MGMT_1` 같은 건 절대 없음
...
| **센서 모델** | **전원 관련 레지스터** | **특징 및 비고** |
| :--- | :--- | :--- |
| **MPU9250** | `PWR_MGMT_1` | Sleep 해제 및 클럭 설정 필수 |
| **MPU6050** | `PWR_MGMT_1` | MPU9250과 거의 동일한 구조 |
| **BMI160** | `CMD`, `PMU_STATUS` | 명령(CMD)을 통해 모드 전환 후 상태 확인 |
| **LIS3DH** | `CTRL_REG1` | ODR(출력 데이터 속도) 설정으로 전원 제어 |
| **BME280** | `CTRL_MEAS`, `CONFIG` | 모드(Normal/Forced) 및 오버샘플링 설정 |

👉 **전원 제어는 다 필요하지만**                <br>
👉 **레지스터 이름 / 주소 / 비트 의미는 전부 다름**
...
```
이라서
**마치 STM32가 0x6B를 “가지고 있는 것처럼” 보이기 때문이야.**

하지만 실제로는:
```objectivec
STM32  ──I2C──▶  MPU9250 내부 메모리(레지스터)
```
STM32는 그냥 **전달자**야.
...
- STM32 = 리모컨
- MPU9250 = 에어컨
- `0x6B (PWR_MGMT_1)` = **에어컨 내부 설정 메뉴**
- `data` = `0x00` = **“전원 켜기 버튼”**

리모컨 안에 에어컨 설정이 있는 게 아니잖아?     <br>
똑같아.

### **이걸 이해했다면 지금 네 머릿속 구조는 이거임**
```yaml
[ STM32 MCU ]
   |
   |  I2C 통신
...
```
이 구조가 딱 떠오르면       <br>
👉 **앞으로 어떤 센서 데이터시트 봐도 안 무너짐**

<br>

### 🤔 **근데 그러면 지금 저 코드가 MPU9250센서 코드인데 그러면 저 MPU9250센서는 원래 저렇게 전원동작을 해야해? 다른센서들은 그런게 없었던거같은데 다른센서도 다 저거 해야해?**
### **MPU9250은 “원래” 저렇게 해야 합니다**
MPU9250(그리고 MPU-6050 같은 InvenSense IMU 계열)은 전원 인가/리셋 후 기본값이 슬립 모드입니다.
- **PWR_MGMT_1 (0x6B)** 기본값이 보통 0x40 (SLEEP 비트=1)이라서, 전원 넣으면 처음엔 슬립 상태입니다.
- 그래서 데이터시트/애플리케이션 노트에서도 “초기화 시 PWR_MGMT_1에 0x00 써서 슬립 해제하라”고 명시합니다.
- 즉, 이 칩을 쓰려면 저 전원/동작 설정을 한 번은 해줘야 정상 동작합니다. “선택 사항”이 아니라 필수 초기화에 가깝습니다.

그래서 “원래 저렇게 전원 동작을 해야 하냐?” → 네, MPU9250은 원래 설계가 저렇게라서 저렇게 해야 합니다.

### 😥 **다른 센서도 다 저거 해야 하냐?**
#### **아니요. 센서마다 다릅니다.**
- MPU9250/MPU-6050 같은 IMU
    - 전원 들어오면 슬립 모드 → PWR_MGMT_1에 0x00 써서 깨우는 코드 필요 (지금 하신 것처럼).
- 다른 I2C 센서들
    - 전원 들어오면 이미 활성 모드로 동작하는 것 많음 → “슬립 해제” 같은 건 없을 수 있음.
    - 대신 측정 범위, 샘플레이트, 필터 설정 같은 다른 레지스터 설정은 할 수 있음.
- 일부 센서
    - 슬립/스탠바이 모드가 있으면, 그런 센서만 “깨우기” 단계가 있고, 나머지는 없을 수 있음.
정리하면, “다른 센서도 다 PWR_MGMT_1 같은 걸 해야 하냐?”가 아니라,      <br>
“그 센서 데이터시트에 초기화/파워 모드 절차가 있냐?”를 보면 됩니다.     <br>
MPU9250은 그 절차 중 하나가 “PWR_MGMT_1에 0x00 쓰기”인 거고, 다른 센서는 그런 레지스터가 아예 없거나, 있더라도 이름/주소/값이 다릅니다.

<br>

### 🤔 **그러면 MPU9250에 동작원리가 어떻게 돼?? 코드가 실행이 되면 저 센서 자체가 그냥 일단 잠을자게되어있어? 코드말고 그냥 센서자체가 원래 그렇게 되어있는건지 궁금해서**
네. **코드가 아니라 센서(칩) 자체 설계입니다.**

### **MPU9250 전원 넣었을 때**
전원을 넣거나 리셋이 걸리면 MPU9250 내부 레지스터가 공장에서 정해둔 기본값으로 설정됩니다.
그중 PWR_MGMT_1 (0x6B) 레지스터의 기본값이 보통 0x40입니다.
- 비트 6 = SLEEP = 1 → 슬립 모드
- 그래서 전원만 넣은 순간부터, 아무 코드 없이도 칩은 이미 “잠든 상태”로 동작합니다.

즉, “코드가 실행되면 잠들게 하는 것”이 아니라, 처음부터 칩이 잠든 상태로 깨어 있는 것이 맞습니다.

### **왜 이렇게 만들어졌나**
- 전력 절약: 전원 인가 직후부터 바로 가속도/자이로를 켜면 전류를 많이 씁니다.

기본을 슬립으로 두면, MCU가 준비될 때까지 전류를 적게 쓸 수 있습니다.
- 제어권을 MCU에 줌: “언제 측정을 시작할지”를 펌웨어에서 정할 수 있게 하려는 설계입니다.
