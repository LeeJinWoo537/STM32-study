...
void MPU9250_Read_Accel_Gyro_DMA(void)
{
    ...
    // 가속도계와 자이로 데이터를 16비트 값으로 변환
    int16_t accelData[3];  // 가속도계 (X, Y, Z)
    int16_t gyroData[3];   // 자이로 (X, Y, Z)

    // 가속도계 데이터
    accelData[0] = (int16_t)((rxData[0] << 8) | rxData[1]);
    accelData[1] = (int16_t)((rxData[2] << 8) | rxData[3]);
    accelData[2] = (int16_t)((rxData[4] << 8) | rxData[5]);

    // 자이로 데이터
    gyroData[0] = (int16_t)((rxData[6] << 8) | rxData[7]);
    gyroData[1] = (int16_t)((rxData[8] << 8) | rxData[9]);
    gyroData[2] = (int16_t)((rxData[10] << 8) | rxData[11]);
